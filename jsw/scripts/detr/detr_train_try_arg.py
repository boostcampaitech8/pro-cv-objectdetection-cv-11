...
##################################
# instances -> bboxes/labels 변환
# ConvertInstancesToAlbuFormat에서 NumPy array로 변환할 때, shape을 [N, 4]로 유지하고 horizontal boxes로 취급하도록 합니다:
class ConvertInstancesToAlbuFormat:
    def __call__(self, results):
        # instances 가져오기
        instances = results.pop('instances')

        # numpy array로 변환 (x, y, w, h)
        results['bboxes'] = np.array([inst['bbox'] for inst in instances], dtype=np.float32)
        results['labels'] = np.array([inst['bbox_label'] for inst in instances], dtype=np.int64)
        
        # BaseBoxes 삭제/무시
        if 'gt_bboxes' in results:
            results.pop('gt_bboxes')
        if 'gt_labels' in results:
            results.pop('gt_labels')
...
cfg.train_dataloader.dataset.pipeline = [
    ...
    dict(type=ConvertInstancesToAlbuFormat),  
...
# data_info = train_ds.get_data_info(0)
# print(data_info)

# from copy import deepcopy
# data_info = deepcopy(train_ds.get_data_info(0))

# # LoadImageFromFile + LoadAnnotations 단계만 적용
# for t in train_ds.pipeline[:2]:
